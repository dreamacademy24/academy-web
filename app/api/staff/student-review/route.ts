...
export async function GET(req:Request){
 ...
 try{
  ...
  const items=review.items.map(item=>{
   ...
   return {...item,name:typeof name==='string'?name:'이름 미등록',reservation:b.reservation_no||b.id,start:b.academy_start,end:b.academy_end,
    candidates:item.candidateIds.map(id=>({id,name:byStudent.get(id)?.name_kr||'이름 미등록',english:byStudent.get(id)?.name_en||''}))};
  });
  return respond({items,issues:review.issues,loadedAt:new Date().toISOString(),bookingCount:bookings.length,studentRowCount:students.length,

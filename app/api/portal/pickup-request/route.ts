...
// 손님 포털 픽드랍 신청 통합 API
// request_type: 'arrival' (입국 픽업) | 'departure' (출국 드랍) | 'extra' (추가 픽드랍)
// GET /api/portal/pickup-request?booking_id=...
//   → { requests: [...] } — 해당 예약의 픽드랍 신청 전체 (모든 타입)
// POST { booking_id, request_type, request_date, request_time, location?, destination, num_people?, flight_info?, notes? }
//   → 신규 행 생성. arrival/departure는 같은 type+booking_id 중복 시 upsert(기존 1건 update)
//   → extra는 매번 새 행 insert
...
const ALLOWED_TYPES = ['arrival', 'departure', 'extra'] as const
...
export async function POST(req: Request) {
  try {
    ...
    if (!ALLOWED_TYPES.includes(request_type as RequestType)) {
      return NextResponse.json({ error: 'request_type must be arrival/departure/extra' }, { status: 400 })
    }

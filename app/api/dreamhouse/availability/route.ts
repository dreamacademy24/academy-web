...
export async function GET(req: Request) {
  try {
    ...
    const { data } = await supabase
      .from('bookings')
      .select('id, accom_type, booking_type, checkin_date, checkout_date, status, seg1_type, seg1_checkin, seg1_checkout, seg2_type, seg2_checkin, seg2_checkout')
      .lt('checkin_date', co)
      .gt('checkout_date', ci)
      .not('status', 'ilike', '%취소%')
    ...
    for (const b of (data || [])) {
      ...
      if (segs.length) {
        ...
        if (rci && rco) ranges.push({ ci: rci, co: rco })
        continue
      }

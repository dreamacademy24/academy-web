...
import CommuteDepositNotice from "@/components/CommuteDepositNotice";
...
const DH:Record<string,P3>={
  ...
  // 보호자 4명 — 확정 규칙(2026-09-01): 추가 1인 비수기 = 1주 27만 / 2주부터 주당 23만, 정가=×1.25 올림, 성수기=×1.125 반올림
  "1-4-1":[3030000,2420000,2720000],
  "1-4-2":[3910000,3120000,3500000],
  "2-4-1":[5230000,4170000,4700000],
  "2-4-2":[6710000,5360000,6040000],
  "3-4-1":[7640000,6100000,6870000],
  "3-4-2":[9750000,7780000,8760000],
  "4-4-1":[9660000,7720000,8700000],
  "4-4-2":[12310000,9840000,11080000],
  "5-4-1":[12020000,9610000,10800000],
  "5-4-2":[15340000,12260000,13780000],
  "6-4-1":[14390000,11500000,12930000],
  "6-4-2":[18370000,14680000,16510000],
  "7-4-1":[16760000,13390000,15060000],
  "7-4-2":[21400000,17100000,19240000],
  "8-4-1":[19120000,15290000,17200000],
  "8-4-2":[24430000,19540000,21980000],
  "9-4-1":[21490000,17180000,19330000],
  "9-4-2":[27460000,21960000,24710000],
  "10-4-1":[23860000,19080000,21470000],
  "10-4-2":[30500000,24390000,27440000],
  "11-4-1":[26230000,20970000,23600000],
  "11-4-2":[33530000,26810000,30170000],
  "12-4-1":[28580000,22860000,25710000],
  "12-4-2":[36540000,29230000,32870000]
};
...
function InvoicePageInner(){
  ...
  const [tab,setTab]=useState<"invoice"|"receipt"|"cert">(initialTab);
  ...
  //  · 단독 리조트 = 총액의 50% · 콤보(드하+리조트) = 드하 100만 + 리조트 구간 50% · 통학 = 최종 원화 청구액 30% · 드하 단독 = 100만
  ...
  const depositAmt=(()=>{
    if(isCommute)return commuteDeposit(fp);
...
@media(max-width:600px){.fw{padding:20px 12px 40px;}.f-row{flex-direction:column;gap:8px;}.it{flex-direction:column;gap:12px;}.iv{padding:24px 12px;}.dr{flex-direction:column;gap:8px;}.ex-row{flex-direction:column;gap:8px;align-items:stretch;}.ex-row .f-group{flex:1!important;}.pb{flex-direction:column;gap:8px;align-items:stretch;}.pb button{width:100%;}.iw{padding:20px 8px 40px;}.is table{display:block;overflow-x:auto;-webkit-overflow-scrolling:touch;}.ba,.bg,.bs,.pp,.psv,.prc,.pbk,.pci,button{min-height:44px;}.fs,.fs-admin{padding:16px 12px;}}
      ...
      <div className="iv" id="guest-invoice">
        ...
        {fp>0&&(
          <div className="is"><div className="ist" style={{color:"#4f46e5",fontSize:"11px",fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase"}}>Payment Schedule</div>
            {isCommute&&<CommuteDepositNotice total={fp} fullPayment={effectiveFullPayment} />}
            {effectiveFullPayment?(
              ...
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"12px 16px",background:"#f0fdf4",border:"1px solid #bbf7d0",borderRadius:8,marginBottom:8}}>
                <span style={{fontWeight:700,color:"#166534"}}>Deposit{isCommute?" (30%)":""} (Booking Confirmation)</span>
  ...
  <div className="iw no-print" style={{paddingTop:24,paddingBottom:0}}>
    <div className="mt" style={{maxWidth:480,margin:"0 auto 8px"}}>
      ...
      {!isCommute&&<button className={`mb${tab==="cert"?" ac":""}`} onClick={()=>setTab("cert")}>🛂 영문 확인서</button>}
    </div>
  </div>

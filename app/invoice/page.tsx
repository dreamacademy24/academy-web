...
export default function InvoicePage(){
  ...
  const [cust,setCust]=useState({name:"",reservationNo:"",reservationDate:"",checkInDate:"",checkInTime:"15:00",checkOutDate:"",checkOutTime:"12:00",packageType:"",people:"",englishName:""});
  const [bill,setBill]=useState({basePrice:0,items:[] as{label:string;price:number;season:string}[]});
  const [discs,setDiscs]=useState<Disc[]>([]);
  const [locals,setLocals]=useState<LC[]>([{id:1,name:"SSP / SSP I card",amount:""},{id:2,name:"드림하우스 보증금",amount:""}]);
  const [ci,setCi]=useState({pickup:"O",drop:"O",flightIn:"",flightOut:"",bedSetting:"",usim:"",houseNo:"",specialRequest:""});
  ...
  const td=discs.reduce((s,d)=>s+(Number(d.amount)||0),0);
  const fp=bill.basePrice-td;
  function addD(){setDiscs([...discs,{id:Date.now(),name:"",amount:0}]);}
  function rmD(id:number){setDiscs(discs.filter(d=>d.id!==id));}
  function upD(id:number,f:string,v:string|number){setDiscs(discs.map(d=>d.id===id?{...d,[f]:v}:d));}
...
.iw{max-width:800px;margin:0 auto;padding:40px 24px 60px;}.iv{background:#fff;padding:48px 40px;box-shadow:0 2px 20px rgba(0,0,0,0.08);}
...
@media(max-width:600px){.f-row{flex-direction:column;gap:12px;}.it{flex-direction:column;gap:12px;}.iv{padding:24px 16px;}.dr{flex-direction:column;gap:8px;}}
  ...
  <div className="fs"><h2>고객 정보</h2>
    <div className="f-row"><div className="f-group"><label className="f-label">예약자명</label><input className="f-input" value={cust.name} onChange={e=>setCust({...cust,name:e.target.value})}/></div><div className="f-group"><label className="f-label">영문이름</label><input className="f-input" placeholder="HONG GILDONG" value={cust.englishName} onChange={e=>setCust({...cust,englishName:e.target.value})}/></div></div>
    <div className="f-row"><div className="f-group"><label className="f-label">예약번호</label><input className="f-input" value={cust.reservationNo} onChange={e=>setCust({...cust,reservationNo:e.target.value})}/></div><div className="f-group"><label className="f-label">예약일</label><input className="f-input" type="date" value={cust.reservationDate} onChange={e=>setCust({...cust,reservationDate:e.target.value})}/></div></div>
    <div className="f-row"><div className="f-group"><label className="f-label">체크인 날짜</label><input className={`f-input${bill.items.length?" auto":""}`} type="date" value={cust.checkInDate} onChange={e=>setCust({...cust,checkInDate:e.target.value})}/></div><div className="f-group"><label className="f-label">체크인 시간</label><input className="f-input" type="time" value={cust.checkInTime} onChange={e=>setCust({...cust,checkInTime:e.target.value})}/></div></div>
    <div className="f-row"><div className="f-group"><label className="f-label">체크아웃 날짜</label><input className={`f-input${bill.items.length?" auto":""}`} type="date" value={cust.checkOutDate} onChange={e=>setCust({...cust,checkOutDate:e.target.value})}/></div><div className="f-group"><label className="f-label">체크아웃 시간</label><input className="f-input" type="time" value={cust.checkOutTime} onChange={e=>setCust({...cust,checkOutTime:e.target.value})}/></div></div>
    <div className="f-row"><div className="f-group"><label className="f-label">패키지 종류</label><input className={`f-input${bill.items.length?" auto":""}`} value={cust.packageType} onChange={e=>setCust({...cust,packageType:e.target.value})}/></div><div className="f-group"><label className="f-label">인원 구성</label><input className={`f-input${bill.items.length?" auto":""}`} value={cust.people} onChange={e=>setCust({...cust,people:e.target.value})}/></div></div>
  </div>

  <div className="fs"><h2>결제 정보</h2>
    {bill.items.length>0?(<div style={{marginBottom:"14px"}}>{bill.items.map((item,i)=><div key={i} style={{fontSize:"13px",color:"#374151",marginBottom:"4px"}}>{item.label} ({item.season}): <strong style={{color:"#1a6fc4"}}>{fmt(item.price)}원</strong></div>)}<div style={{fontSize:"14px",fontWeight:800,marginTop:"8px"}}>합계: {fmt(bill.basePrice)}원</div></div>):(<div className="f-row"><div className="f-group"><label className="f-label">패키지 금액 (원)</label><input className="f-input" type="number" value={bill.basePrice||""} onChange={e=>setBill({basePrice:Number(e.target.value),items:[]})}/></div></div>)}
    ...
    {locals.map(c=><div className="dr" key={c.id}><div className="f-group"><input className="f-input" placeholder="항목명" value={c.name} onChange={e=>upL(c.id,"name",e.target.value)}/></div><div className="f-group"><input className="f-input" placeholder="금액 (예: 7,000 pesos)" value={c.amount} onChange={e=>upL(c.id,"amount",e.target.value)}/></div><button className="bs br" onClick={()=>rmL(c.id)}>삭제</button></div>)}
    <button className="bs bd" onClick={addL}>+ 현지 지불 항목 추가</button>
  </div>

  <div className="fs"><h2>체크인 정보</h2>
    <div className="f-row"><div className="f-group"><label className="f-label">픽업</label><select className="f-select" value={ci.pickup} onChange={e=>setCi({...ci,pickup:e.target.value})}><option value="O">O</option><option value="X">X</option></select></div><div className="f-group"><label className="f-label">드롭</label><select className="f-select" value={ci.drop} onChange={e=>setCi({...ci,drop:e.target.value})}><option value="O">O</option><option value="X">X</option></select></div></div>
    <div className="f-row"><div className="f-group"><label className="f-label">항공편 (IN)</label><input className="f-input" value={ci.flightIn} onChange={e=>setCi({...ci,flightIn:e.target.value})}/></div><div className="f-group"><label className="f-label">항공편 (OUT)</label><input className="f-input" value={ci.flightOut} onChange={e=>setCi({...ci,flightOut:e.target.value})}/></div></div>
    <div className="f-row"><div className="f-group"><label className="f-label">베드 세팅</label><input className="f-input" value={ci.bedSetting} onChange={e=>setCi({...ci,bedSetting:e.target.value})}/></div><div className="f-group"><label className="f-label">USIM</label><input className="f-input" value={ci.usim} onChange={e=>setCi({...ci,usim:e.target.value})}/></div></div>
    <div className="f-row"><div className="f-group"><label className="f-label">하우스 번호</label><input className="f-input" value={ci.houseNo} onChange={e=>setCi({...ci,houseNo:e.target.value})}/></div></div>
    <div className="f-group" style={{marginTop:"4px"}}><label className="f-label">특별 요청사항</label><textarea className="f-textarea" value={ci.specialRequest} onChange={e=>setCi({...ci,specialRequest:e.target.value})}/></div>
  </div>

  <button className="bg" onClick={gen}>인보이스 생성</button>
  <a href="/" className="bl">← 홈으로 돌아가기</a>
  </div>):(
  <div className="iw">
    <div className="iv">
      <div className="it"><div><div className="il">DREAM COMPANY</div><div className="ils">Philippines</div></div><div className="itr"><h1>INVOICE</h1>{cust.reservationNo&&<p>No. {cust.reservationNo}</p>}</div></div>
      <div className="is"><div className="ist">Customer Information</div><table className="tb"><tbody>
        ...
        <tr><td className="lb">예약번호</td><td>{cust.reservationNo}</td><td className="lb">예약일</td><td>{cust.reservationDate}</td></tr>
        <tr><td className="lb">체크인</td><td>{cust.checkInDate} {cust.checkInTime}</td><td className="lb">체크아웃</td><td>{cust.checkOutDate} {cust.checkOutTime}</td></tr>
      ...
      {locals.filter(c=>c.name&&c.amount).length>0&&<table className="tb" style={{marginTop:"12px"}}><thead><tr><th style={{width:"60%"}}>현지 지불 항목</th><th style={{width:"40%",textAlign:"right"}}>금액</th></tr></thead><tbody>{locals.filter(c=>c.name&&c.amount).map((c,i)=><tr key={i}><td>{c.name}</td><td style={{textAlign:"right"}}>{c.amount}</td></tr>)}</tbody></table>}</div>
      <div className="is"><div className="ist">Check In Details</div><table className="tb"><tbody>
        <tr><td className="lb">픽업</td><td>{ci.pickup}</td><td className="lb">드롭</td><td>{ci.drop}</td></tr>
        <tr><td className="lb">항공편 (IN)</td><td>{ci.flightIn}</td><td className="lb">항공편 (OUT)</td><td>{ci.flightOut}</td></tr>
        <tr><td className="lb">베드 세팅</td><td>{ci.bedSetting}</td><td className="lb">USIM</td><td>{ci.usim}</td></tr>
        <tr><td className="lb">하우스 번호</td><td colSpan={3}>{ci.houseNo}</td></tr>
        {ci.specialRequest&&<tr><td className="lb">특별 요청</td><td colSpan={3} style={{whiteSpace:"pre-wrap"}}>{ci.specialRequest}</td></tr>}
      </tbody></table></div>

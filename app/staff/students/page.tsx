...
type Roster={isAdmin:boolean;visits:Visit[];teachers:{id:string;name:string}[];sourceCount?:number;pendingStudents?:SourceStudent[];selection?:{name:string;bookingId:string;sourceIndex:number;visitId:string|null}};
...
export default function StudentCare(){
 ...
 const [embedded,setEmbedded]=useState(false),[selectionQuery,setSelectionQuery]=useState('');
 const load=useCallback(async()=>{
  setBusy(true);setError('');setData(null);setAuth(false);setPending(null);
  const params=new URLSearchParams(window.location.search),scope=new URLSearchParams();
  for(const key of ['bookingId','sourceIndex','sourceId'])if(params.has(key))scope.set(key,params.get(key)||'');
  setEmbedded(params.get('embedded')==='1');setSelectionQuery(scope.toString());
  const endpoint='/api/staff/students'+(scope.size?'?'+scope.toString():'');
  try{const r=await fetch(endpoint,{cache:'no-store'});const d=await r.json();if(!r.ok){setAuth(r.status===401);throw new Error(d.error||'Unable to load student care.');}setData(d);
   if(d.isAdmin){
    const syncResponse=await fetch(endpoint,{method:'POST'});const result=await syncResponse.json();
    ...
    else{setData(result.roster);if(result.sync.linked&&!scope.size)setMessage(`일치하는 방문 ${result.sync.linked}건을 자동 연결했습니다.`);else if(result.sync.busy)setMessage('다른 화면에서 자동 연결 중입니다. 잠시 후 Refresh를 눌러주세요.');}
   }
  }
 ...
 const reviewHref='/staff/student-review'+(selectionQuery?'?'+selectionQuery:'');
 return <main className={styles.main}>
  <header className={styles.heading}><h1>{data?.selection?`${data.selection.name} · 학생케어`:admin?'학생케어':'My Students'}</h1><div className={styles.headingActions}>{!embedded&&<Link href={admin?'/admin/students?tab=attending':'/admineng/hub'}>{admin?'학생관리':'Teacher Hub'}</Link>}<button disabled={busy||saving} onClick={()=>{setMessage('');void load();}}>{busy?(admin?'불러오는 중…':'Loading…'):(admin?'↻ 새로고침':'↻ Refresh')}</button></div></header>
  {auth&&<button disabled={busy} onClick={login}>Sign in again / 다시 로그인</button>}
  {error&&<p className={styles.error} role="alert">{error}</p>}
  {message&&<p className={styles.notice} role="status">{message}</p>}
  {data&&<>
   {!selected&&<div className={styles.overview}><dl className={styles.metrics}>{admin?<><div><dt>기존 명부</dt><dd>{(data.sourceCount||0).toLocaleString()}<small>건</small></dd></div><div><dt>연결된 방문</dt><dd>{data.visits.length.toLocaleString()}<small>건</small></dd></div><div className={styles.reviewMetric}><dt>확인 필요</dt><dd>{sourceVisits.length.toLocaleString()}<small>건</small></dd></div></>:<><div><dt>Students</dt><dd>{new Set(data.visits.map(v=>v.learner_id)).size}</dd></div><div><dt>Visits</dt><dd>{data.visits.length}</dd></div></>}</dl>{admin&&<Link className={styles.reviewLink} href={reviewHref}>기록 검토 →</Link>}</div>}
   {!selected&&<div className={styles.toolbar}><label className={styles.search}>{admin?'학생 검색':'Search students'}<input type="search" value={query} onChange={e=>{setQuery(e.target.value);setPage(0);}} placeholder={admin?'한글 / 영문 이름':'Korean / English name'}/></label>{admin&&<div className={styles.filters}><label className={styles.check}><input type="checkbox" checked={onlyUnassigned} onChange={e=>{setOnlyUnassigned(e.target.checked);setOnlyReview(false);setPage(0);}}/>담당자 미배정</label><label className={styles.check}><input type="checkbox" checked={onlyReview} onChange={e=>{setOnlyReview(e.target.checked);setOnlyUnassigned(false);setPage(0);}}/>확인 필요</label></div>}</div>}
   {!selected&&!!allVisits.length&&<div className={styles.results} role="status"><span>{admin?`검색 결과 ${visits.length.toLocaleString()}건`:`${learners.length} students · ${visits.length} visits`}</span>{learners.length>25&&<span>{currentPage+1} / {lastPage+1}</span>}</div>}
   {!allVisits.length&&<section className={styles.empty}><h2>{admin?'등록된 학생이 없습니다':'No assigned students yet'}</h2></section>}
   ...
   <div className={styles.list}>{learners.slice(currentPage*25,(currentPage+1)*25).map(id=>{
    ...
    return <article key={id} className={styles.student}><header><span className={styles.avatar} aria-hidden="true">{(student.name_en||student.name_kr).slice(0,1)}</span><div><h2>{student.name_en||student.name_kr}</h2><p>{student.name_en&&<>{student.name_kr} · </>}{student.reason?'기존 명부':`${periods.length} ${admin?'방문':'visits'}`}</p></div></header>
     {periods.map(v=><section key={v.id} className={styles.visit} aria-label={`${student.name_kr} ${v.start_date}`}>
      <div className={styles.period}><h3>{v.start_date} → {v.end_date}</h3><span className={v.reason?styles.reviewBadge:styles.visitBadge}>{v.reason?'확인 필요':admin?(v.linkMethod==='automatic'?'자동 연결':'연결 완료'):'Care team'}</span></div>
      {v.reason?<div className={styles.reviewRow}><p>{v.reason}</p><Link href={reviewHref}>기록 확인 →</Link></div>:<>
      <div className={styles.assignmentRow}><div className={styles.teachers}>{v.assignments.filter(a=>a.active).map(a=><div key={a.teacherId}><span>{a.name}{!a.available&&(admin?' · 계정 비활성':' · unavailable')}</span>{admin&&<button disabled={saving||!!pending} onClick={()=>choose(v,a.teacherId,false,a.name)}>해제</button>}</div>)}{v.assignments.every(a=>!a.active)&&<p>{admin?'담당자 미배정':'No teacher assigned'}</p>}</div>

...
export default async function Page() {
  ...
  const staffPreview = access.status === 'staff';
  return <main className={styles.entry} data-learning-access={staffPreview ? 'staff-preview' : 'coming-soon'}>
    ...
    <div className={styles.choices}>
      ...
      <article className={`${styles.card} ${styles.learningSoon}`} aria-labelledby="learning-mode-title">
        ...
        <div className={styles.cardBody}><span className={styles.category}>LEARNING · 영어 학습</span><h2 id="learning-mode-title">학습모드</h2>
          {staffPreview ? <><p>드림이와 고래상어가 함께하는<br/>영어 모험을 미리 체험해보세요.</p><Link href="/learn" className={`${styles.cta} ${styles.previewStart}`}>체험 시작 <span lang="en">Preview</span><Icon name="arrow"/></Link><p className={styles.previewNote}>직원 전용 체험 · Staff preview</p></> : <><p>드림이와 고래상어가 함께하는<br/>새로운 영어 모험을 준비하고 있어요.</p><span className={styles.comingSoon}>준비 중</span></>}
        </div>
      </article>
    </div>
    <p className={styles.switchNote}>연수 신청과 예약 확인은 게스트 모드에서 이용해 주세요.</p>
    {!staffPreview && <p className={styles.staffEntry}><Link href="/login?next=%2Fdream-app">직원 체험 로그인 · Staff sign in</Link></p>}

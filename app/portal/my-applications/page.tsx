      ...
      {tab === "fieldtrip" && (
        <div>
          {data.fieldtrip.length === 0 ? (
            ...
            // 형제 합치기: 프로그램 구성(date 토큰)과 상태가 완전히 같으면 카드 1장으로
            const normTokens = (r: AnyRow) => String(r.date || "").split(",").map(t => t.trim()).filter(Boolean).sort().join(",");
            const groups = new Map<string, AnyRow[]>();
            data.fieldtrip.forEach((r: AnyRow) => {
              const k = normTokens(r) + "|" + String(r.status || "");
            ...
            return Array.from(groups.values()).map(grp => {
              const r = grp[0];
              const meta = statusMeta(String(r.status || ""));
              const names = grp.map(x => String(x.name || "").trim()).filter(Boolean);
              const title = names.length ? `${names.join(", ")} · 애프터스쿨/필드트립` : "애프터스쿨/필드트립";

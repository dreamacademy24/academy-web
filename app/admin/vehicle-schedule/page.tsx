...
type Override={driver_id?:string|null;time?:string;note?:string;vehicle_name?:string;teacher_name?:string};
  ...
  try{
   ...
   try{
    for(const target of targets){
     const st={overrides:{[target.id]:{driver_id:editor.driver_id||null,time:editor.time,note:editor.applicants?target.note||'':editor.note||'',vehicle_name:editor.vehicle_name||'',teacher_name:editor.teacher_name||''}}};
   ...
   setEditor(null);setMessage(targets.length>1?'탑승 신청 '+targets.length+'건의 차량·기사·담당 티쳐를 저장했습니다.':'배정 내용이 저장되었습니다.');
   await load();

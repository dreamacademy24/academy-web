...
export default function VehicleSchedulePage(){
 ...
 const [editor,setEditor]=useState<VehicleRow|null>(null),[saving,setSaving]=useState(false),[sources,setSources]=useState(false);
 ...
 const rows=useMemo(()=>{
  ...
  return groupShuttleRuns(items).filter(m=>(tab==='all'||vehicleCategory(m)===tab)&&(!query||[m.guest,m.location,m.destination,m.flight_info,m.driver_name,drivers.find(d=>d.id===m.driver_id)?.name].join(' ').toLowerCase().includes(query.toLowerCase())))
 ...
 const openEditor=(m:VehicleRow)=>{setMessage('');setEditor({...m});};
 ...
 const save=async()=>{
  ...
  try{
   const targets=editor.applicants||[editor];
   let savedCount=0;
   try{
    for(const target of targets){
     const st={overrides:{[target.id]:{driver_id:editor.driver_id||null,time:editor.time,note:editor.applicants?target.note||'':editor.note||''}}};
     const response=await fetch('/api/admin/vehicle-schedule',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({date:editor.date,state:st,onlyId:target.id})});
     const j=await response.json();if(!response.ok)throw new Error(j.error||'저장 실패');
     savedCount++;setStates(prev=>({...prev,[editor.date]:j.state}));
    }
   }catch(e){throw new Error(savedCount+'/'+targets.length+'건 저장됨. '+(e as Error).message+' 입력을 유지했습니다. 다시 저장하면 나머지도 반영됩니다.');}
   setEditor(null);setMessage(targets.length>1?'셔틀 신청 '+targets.length+'건에 같은 시간·기사를 배정했습니다.':'배정 내용이 저장되었습니다.');
   await load();
 ...
 return <main className="vehicle-page">
  ...
  {error?<div className="vehicle-empty" role="alert">{error}<button onClick={()=>void load()}>다시 불러오기</button></div>:loading?<div className="vehicle-empty" role="status">일정을 불러오고 있습니다…</div>:<div className="vehicle-table-wrap"><table className="vehicle-table"><thead><tr><th>구분</th><th>{pending?'운행일 / 시간':'시간'}</th><th>운행 내용</th><th>탑승</th><th>기사</th><th><span className="vehicle-sr">상세 보기</span></th></tr></thead><tbody>
   {rows.map(m=>{const source=sourceOf(m),isOpen=expanded===m.id,board=m.commuteDetails,needed=m.applicants?m.applicants.some(needsDispatch):needsDispatch(m),driver=m.mixedDrivers?'배정 확인':m.driver_name||drivers.find(d=>d.id===m.driver_id)?.name||(m.driver_id?'기존 배정 기사':'미배정');return <Fragment key={m.id}><tr className={isOpen?'expanded':''}>
    ...
    <div>{m.applicants?<><b>신청자 명단 · 총 {m.num_people}명</b><ul>{m.applicants.map(p=><li key={p.id}><strong>{p.guest}</strong> · {p.num_people}명{p.note&&<p>{p.note}</p>}{m.mixedDrivers&&<small>기사: {p.driver_name||drivers.find(d=>d.id===p.driver_id)?.name||'미배정'}</small>}</li>)}</ul><p>같은 날짜·시간·노선의 신청을 함께 표시합니다.</p></>:board?<><b>탑승 명단</b><ul>{board.cards.map((card,i)=><li key={i}><strong>{card.addr}</strong> ({card.count||'0'}) {card.names}</li>)}</ul>{!!board.absent?.length&&<p className="vehicle-alert">결석·미탑승 메모: {board.absent.join(' · ')}</p>}</>:<><b>{m.guest}</b><p>{m.note||'등록된 운행 메모가 없습니다.'}</p></>}{m.auto&&<p>항공 정보로 만든 초안입니다. 실제 차량 시간과 운행 여부를 확인해주세요.</p>}</div>

...
.hub-w{max-width:760px;margin:0 auto;padding:60px 24px;}
...
.hub-grid{display:grid;grid-template-columns:1fr 1fr 1fr;gap:20px;margin-bottom:36px;}
.hub-card{border-radius:16px;padding:36px 20px;text-align:center;cursor:pointer;border:2px solid transparent;transition:all 180ms;}
.hub-card:hover{transform:translateY(-3px);}
.hub-card .ic{font-size:40px;margin-bottom:14px;}
.hub-card h2{font-size:17px;font-weight:800;margin-bottom:8px;}
.hub-card p{font-size:12px;line-height:1.6;opacity:0.85;}
...
.card-green{background:#16a34a;color:#fff;box-shadow:0 6px 24px rgba(22,163,74,0.25);}.card-green:hover{box-shadow:0 10px 36px rgba(22,163,74,0.35);}
.hub-footer{text-align:center;}
.hub-link{color:#6b7c93;font-size:13px;font-weight:600;text-decoration:none;}.hub-link:hover{color:#1a6fc4;}
@media(max-width:600px){.hub-grid{grid-template-columns:1fr;}.hub-w{padding:40px 16px;}}
    `}</style>
    <div className="hub-w">
      <div className="hub-h">
        <h1>드림아카데미 관리자</h1>
      ...
      <div className="hub-grid">
        ...
        <div className="hub-card card-green" onClick={() => router.push("/invoice")}>
          <div className="ic">📝</div>
          <h2>인보이스 생성</h2>
          <p>새 인보이스 직접 작성</p>
        </div>
      ...
      <div className="hub-footer">
        <a className="hub-link" href="/guide">직원 가이드 보기 →</a>
      </div>
    </div>
  </>);
}

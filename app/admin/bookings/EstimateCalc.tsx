...
/* ── 가격 테이블 (invoice/page.tsx 동일, [정가, 비수기, 성수기]) ── */
...
const seasonLabel:Record<Season,string>={off:"비수기",peak:"성수기"};
...
function pickPrice(e:P3,s:Season):number{ return s==="off"?e[1]:e[2]; }
function won(n:number){return n.toLocaleString("ko-KR")+"원";}

interface ExtraItem{name:string;amount:number;}
interface PlanState{
  accom:AccomType; roomType:string; weeks:number; season:Season;
  parents:number; kids:number;
  extras:ExtraItem[]; discounts:ExtraItem[];
}

const defaultPlan=(accom:AccomType="dreamhouse"):PlanState=>({
  accom, roomType:accom==="dreamhouse"?"":"디럭스", weeks:4, season:"off",
...
function calcPlan(p:PlanState){
  ...
  const extraSum=p.extras.reduce((s,x)=>s+x.amount,0);
  const discountSum=p.discounts.reduce((s,x)=>s+x.amount,0);
  return {base,seasonPrice,extraSum,discountSum,subtotal:seasonPrice+extraSum,final:seasonPrice+extraSum-discountSum};
}
...
export default function EstimateCalc(){
  ...
  function up(setter:React.Dispatch<React.SetStateAction<PlanState>>,patch:Partial<PlanState>){
    setter(prev=>{
      ...
      if(patch.accom){
        next.roomType=patch.accom==="dreamhouse"?"":"디럭스";
  ...
  const r1=calcPlan(p1), r2=calcPlan(p2);
    ...
    return(
      <div style={{flex:1,minWidth:300,background:"#fff",borderRadius:12,padding:20,border:"1px solid #e2e8f0"}}>
        ...
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          ...
          <label style={{flex:1}}><span style={lbl}>시즌</span>
            <select style={{...sel,background:plan.season==="peak"?"#fef2f2":"#ecfdf5",fontWeight:600,color:plan.season==="peak"?"#dc2626":"#059669"}} value={plan.season} onChange={e=>up(setter,{season:e.target.value as Season})}>
              <option value="off">비수기</option><option value="peak">성수기</option>
            </select></label>
        </div>
    ...
    return(
      <div style={{flex:1,background:"#fff",borderRadius:12,padding:24,border:"1px solid #e2e8f0"}}>
        ...
        <div style={{textAlign:"center",marginBottom:16,paddingBottom:14,borderBottom:"2px solid #1a6fc4"}}>
          ...
          <div style={{fontSize:13,fontWeight:600,color:"#1a6fc4"}}>{planName(plan)} · {plan.weeks}주</div>
          <div style={{marginTop:6}}>
            <span style={{display:"inline-block",padding:"2px 10px",borderRadius:10,fontSize:11,fontWeight:700,background:plan.season==="peak"?"#fef2f2":"#ecfdf5",color:plan.season==="peak"?"#dc2626":"#059669",border:plan.season==="peak"?"1px solid #fecaca":"1px solid #a7f3d0"}}>{seasonLabel[plan.season]}</span>
          </div>
          <div style={{fontSize:12,color:"#6b7c93",marginTop:6}}>보호자 {plan.parents}명 + 아이 {plan.kids}명</div>
        </div>

        {/* 시즌 가격 (정가 역할) */}
        <div style={{display:"flex",justifyContent:"space-between",padding:"10px 0",fontSize:14}}>
          <span style={{color:"#6b7c93"}}>{seasonLabel[plan.season]} 가격</span>
          <span style={{fontWeight:600}}>{won(r.seasonPrice)}</span>
        </div>

        {/* 추가항목 */}
        {hasExtras&&plan.extras.filter(e=>e.name&&e.amount).map((e,i)=>(
        ...
        {/* 구분선 (추가항목 있을때) */}
        {hasExtras&&<div style={{borderTop:"1px dashed #e2e8f0",margin:"8px 0"}}/>}

        {/* 할인항목 */}
        {hasDiscounts&&plan.discounts.filter(e=>e.name&&e.amount).map((e,i)=>(
          <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",fontSize:13}}>
            <span style={{color:"#6b7c93"}}>{e.name}</span>
        ...
        {/* 구분선 (할인항목 있을때) */}
        {hasDiscounts&&<div style={{borderTop:"1px dashed #e2e8f0",margin:"8px 0"}}/>}

        {/* 합계 */}
        <div style={{borderTop:"2px solid #1a1a2e",marginTop:10,paddingTop:12}}>
          {(hasExtras||hasDiscounts)&&(
          ...
          <div style={{display:"flex",justifyContent:"space-between",padding:"10px 0",alignItems:"center"}}>
            <span style={{fontSize:16,fontWeight:800,color:"#1a1a2e"}}>💛 할인가</span>
            <span style={{fontSize:24,fontWeight:800,color:"#1a6fc4"}}>{won(r.final)}</span>
          </div>
        </div>
      </div>
    );
    ...
    <>
      ...
      <div id="estimate-result" ref={resultRef} style={{background:"#fff",borderRadius:14,padding:36,boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
        ...
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{fontSize:24,fontWeight:800,color:"#1a6fc4",letterSpacing:"-0.02em",marginBottom:2}}>Dream Academy</div>
          <div style={{fontSize:14,fontWeight:700,color:"#1a1a2e",marginBottom:4}}>맞춤 견적서</div>
          <div style={{fontSize:11,color:"#94a3b8"}}>발행일: {today}</div>
        </div>
        ...
        <div style={{padding:"14px 20px",background:"#f8fafc",borderRadius:10,textAlign:"center",fontSize:12,color:"#6b7c93",lineHeight:1.8}}>
          ※ 위 금액은 정가 기준이며, 할인 적용된 실제 금액입니다.<br/>
          문의: 카카오톡 @드림아카데미
        </div>
      </div>

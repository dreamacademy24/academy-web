                          ...
                          {week.map((day,di)=>{
                            ...
                            return (
                              <td key={di} className={`cal-cell${inMonth?"":" out-month"}`}>
                                ...
                                {startList.map(s=>{const isKinder=s.grade==="킨더";const age=getStudentAge(s);const ageStr=age?`${age}y`:"-";const ws=new Date(wsStr+'T00:00:00').getTime();const es=s.academyStart?new Date(s.academyStart+'T00:00:00').getTime():ws;const weekNum=Math.max(1,Math.floor((ws-es)/(7*24*60*60*1000))+1);return (<div key={`s${s.key}`} className="cal-stu-in" title={`${s.korName||""} ${s.engName||""}`.trim()} style={{fontSize:11}}>+ {isKinder&&<span style={{color:"#1a1a2e",fontWeight:800}}>K</span>}{s.korName||""}/{s.engName||""}/{ageStr}/{weekNum}w</div>);})}
                                {endList.map(s=>{const isKinder=s.grade==="킨더";const age=getStudentAge(s);const ageStr=age?`${age}y`:"-";const ws=new Date(wsStr+'T00:00:00').getTime();const es=s.academyStart?new Date(s.academyStart+'T00:00:00').getTime():ws;const weekNum=Math.max(1,Math.floor((ws-es)/(7*24*60*60*1000))+1);return (<div key={`e${s.key}`} className="cal-stu-out" title={`${s.korName||""} ${s.engName||""}`.trim()} style={{fontSize:11}}>- {isKinder&&<span style={{color:"#1a1a2e",fontWeight:800}}>K</span>}{s.korName||""}/{s.engName||""}/{ageStr}/{weekNum}w</div>);})}
                              </td>
                            );
                          })}
                        </tr>

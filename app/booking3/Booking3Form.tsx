...
import OnlineClassRules from '@/components/OnlineClassRules';
      ...
      {receipt ? <section className={s.success}>
        ...
        <div className={s.grid}>
          <section className={s.card}>
            ...
            {step === 1 && <><div className={s.fields}>{field('student', '학생 한글 이름', '예: 김드림', true)}{field('englishName', '학생 영문 이름', '예: Dream Kim', true)}{field('birthYear', '출생연도', '예: 2018', true)}<label className={s.field}>영어 수준 · 레벨<select value={form.level} onChange={e => change('level', e.target.value)}><EnglishLevelOptions value={form.level} /></select></label>{field('guardian', '보호자 이름', '연락받으실 보호자', true)}{field('phone', '보호자 연락처', '010-0000-0000', true, 'tel')}{field('email', '이메일 (선택)', '안내받을 이메일', false, 'email')}{field('referral', '소개해 주신 분 (선택)', '담당자가 확인 후 혜택 안내')}</div><label className={s.field}>상담·요청 사항<textarea rows={4} maxLength={1500} value={form.notes} onChange={e => change('notes', e.target.value)} placeholder="아이의 학습 경험이나 일정 관련 요청을 알려주세요." /></label></>}
            {step === 2 && <>
            ...
            {step === 3 && <>
              <dl className={s.review}><div><dt>학생</dt><dd>{form.student} · {form.englishName} ({form.birthYear}년)</dd></div><div><dt>영어 수준</dt><dd>{englishLevelLabel(form.level)}</dd></div><div><dt>연락처</dt><dd>{form.guardian} · {form.phone}</dd></div><div><dt>희망 일정</dt><dd>{form.startDate}부터<br />{form.days.map(d => `${d} ${form.dayTimes[d]}`).join(' · ')}</dd></div><div><dt>등록 내용</dt><dd>{form.months}개월 · 주 {form.weekly}회 · 총 {quote.totalSessions}회</dd></div></dl>
              ...
              <div className={s.rules}><OnlineClassRules /><h3>수업 준비·선생님 배정</h3><ul><li>수업 당일 개별 연락은 드리지 않습니다. 일정과 링크를 확인하고 시작 5분 전 접속해주세요.</li><li>선생님은 시간과 레벨에 맞춰 배정하며 현지 연수 담당 선생님과 다를 수 있습니다.</li><li>소개 혜택은 담당자가 확인 후 별도 적용합니다. 접수만으로 결제·시간 배정이 확정되지 않습니다.</li></ul><button type="button" className={s.textButton} onClick={() => setGuide(true)}>화상영어 수업 안내·취소·환불 규정 보기 ↗</button></div>

...
## 2026-07-07 저녁 — 잔여 개선 묶음
- ✅ 전체 업무 표 글씨 확대 14.5px + 직원 스트립/칩/뱃지 확대 (10b680c)
- ✅ 튜터 "타임·금액 불일치" 홈 위젯 — end_date 지난(종료) 수업 제외 (10b680c)
- ✅ 주간 체크리스트 — 달력 칸 "+N" 잘림 제거·전체 항목 표시·글씨 12px, 요일 상세 밴드 기본 전부 펼침, 칸 클릭=스크롤 이동+하이라이트 (30e6d9f)
- ✅ 룸 캘린더 모달 — "룸 변경" 셀렉트 기본값 placeholder("— 새 룸 선택 —"), 현재 룸 (현재) 표시, 미선택/목록 밖 룸 오인(b17L4→b13L10) 방지 (c165df4)
- 🔴 대기: 박경숙(DA-2026-02-06 접수, 콤보 DH 8/1~8/8 1주 + JP 8/8~8/22 2주) — 예약 데이터의 주수/구간이 영수증과 다르게 기록된 듯, DH=1주/JP=2주로 수정 필요. Chrome 연결 끊겨 DB 확인 못함 — 다음 접속 시 bookings의 dh_weeks/jp_weeks/seg1·seg2 확인 후 보정

## 2026-07-07 밤 — 드림하우스 오버부킹 3중 안전장치 (f7c8ffe)
1. /api/dreamhouse PATCH 룸 변경: 같은 룸 기간 겹침(활성, 당일 전환 제외, 콤보 DH seg 기준) 시 409 room_conflict 반환 → 모달에서 겹치는 예약 목록 confirm 후 force:true로만 강제 저장
2. 룸 캘린더 상단 🚨 오버부킹 배너: 월과 무관하게 전체 예약 기준 겹침 페어 나열(누구↔누구·겹침 기간), 클릭=해당 예약 모달
3. 직원업무 홈 확인필요에 "🚨 드림하우스 룸 중복" 카드 (_homeRoomConflictsHtml, kOps1 합산, 클릭=룸 캘린더)
- 미검증: Chrome 연결 끊김으로 라이브 확인 못함 (구문검사 통과)
- b17L4 잔존 예약 확인 요청 대기: 지금까지 파악된 b17L4 배정 = 오초희(7월 겹침 조회에서 발견), 정선희(DA-20260508-583098, 2027-01-02~02-13) — 룸 캘린더 ⚠b17L4 컬럼에서 전체 확인 가능
- 박경숙 콤보 주수 보정도 Chrome 복구 대기

## 2026-07-07 심야 — 만실 차단 + 박경숙 보정 + 화면 잘림
- ✅ 접수 단계 만실 차단 (1b98d33): /api/dreamhouse/availability (14룸 기준 날짜별 점유, 미배정 포함, [ci,co) 구간, 콤보 DH seg만) — /booking·/booking2(드하 관련 유형) 만실 날짜 있으면 "관리자에게 확인" 알림+접수 차단, 어드민 신규예약은 경고+선택 진행
...
## 2026-07-07 마무리 잔여 수정
- 학생 날짜 불일치 경고: 종료·체크아웃 지난 과거 예약 자동 제외 (39f317c — 한지운 케이스)
- 식단 미리보기 요일: OCR 인식값 대신 실제 날짜로 계산 (595f021 — 7/6 일→월 밀림)
- 체크리스트 고객 자동표시: 취소 예약 + 룸 없는 통학형 제외 (949310c — 통학형은 드하 룸 있을 때만 체크인/아웃 관리 대상)
- 메이 확인: 오초희 b17L4=가예약 유지, 제이파크 조식 메일 발송함, 부분납 6명=정상(잔금 관리)
- 대기: 김이안 날짜 확인, "3주전 고객" 항목 자동표시 옵션 변경, 주간 체크리스트 디자인 개편(시안 A 아코디언/B 탭 — 메이 선택 대기), 동명이인 나머지 쌍(도유민·장보운) 결정

## 2026-07-07 밤 2차 (54a803f) — 업무 시스템 보강 묶음
- 관리자 전체보기(1a567d1): Vercel 웹훅 누락으로 미배포 상태였음 → 54a803f 푸시로 함께 배포. 검증: 전체 업무 15행(미공유 Candice·Jamie 업무 포함) 표시 ✓
- **첨부 저장 버그(중요)**: 업무 모달(handleTMF)·빠른작업(handleQAF)이 파일 내용 없이 {name,size}만 저장하던 오래된 버그 → base64로 실제 저장 + 선택 시 썸네일. ⚠️ 기존 깨진 첨부(메이 스크린샷 등)는 내용이 아예 없어 복구 불가 — 다시 첨부 필요
- **created_by**: staff_tasks.created_by 추가(DDL 완료). 생성 경로 3곳(모달/빠른추가/빠른작업)에 기록. 가시성: 내가 만든 업무는 담당자가 남이어도 내 화면(전체업무·내업무)에 보임 + 내업무 아이템에 "→담당자" 보라 뱃지
- **내 업무 홈 3컬럼**: 타임라인 | 오늘 체크리스트(홈과 동일 위젯, loadDailyCheck targetId 파라미터화) | 보드. 1500px 이하 체크리스트 하단 풀폭, 1150px 이하 1열. 본인 보드일 때만 표시. 검증 ✓

## 2026-07-07 — 지시→완료 확인 루프 완성 (1761f0e)
- 업무 완료 토글(qTog) 시 만든 사람(createdBy)에게 알림: "✅ OO님이 완료했어요: 업무명" (본인 완료는 제외)
- 흐름 완성: A가 B에게 업무 생성 → A 내업무에 "→B" 뱃지로 유지 → B 완료 → 같은 업무라 A 보드도 완료 칸 이동 + A에게 빨간 점 알림
- ⚠️ created_by는 오늘 이후 생성 업무부터 기록 — 기존 업무는 지시자 정보 없음

## 2026-07-07 — 내 업무 헤더 통합 (9d64317, 6a522cd)
- 제목("내 업무 · 개인 업무 보드")을 탭 줄(홈/체크리스트/학생/달력/결재) 왼쪽 인라인으로
- 빈 헤더 줄(.topbar) 제거: emp 페이지에서 topbar 숨기고 .topbar-right 버튼들(관리자홈/공지추가/⚙/업무추가)을 empSubNav 오른쪽으로 DOM 이동, 다른 페이지 가면 원위치 복원
- 라이브 검증: 탭 줄 = [내 업무·부제 | 탭들 | ...버튼들] 한 줄 통합, 홈 복귀 시 원상복구 ✓

## 2026-07-07 — 식단 주간 명단 마감 시스템 (c8d4a7d) + 사이드바
- 사이드바: "식단 관련 업무" 대분류 분리(식단 관련 업무+모리칸/식단생성) → 리조트 아래 배치 (2ccaf3e, a7cccf5)
- 주간 명단 마감: [✅ 최종 확인·이번 주 마감] → app_settings meal_week_close {주월요일:{by,at}} 저장, 마감 후 표 수정 잠금(pointer-events)+마감자·시각 표시+[마감 해제]. [🍽 식단생성으로 →] 버튼은 마감 후에만 활성 (마감→식단생성 워크플로우)
- 이상 자동 비고: 비표준(토요일 아님) 체크인/퇴소를 행에 "⚠ 금요일 체크인" 빨간 칩으로 자동 표시 (통학형 제외), 입퇴소 예고에도 요일+⚠ 병기. 라이브 검증 ✓ (김애나 JUL10 금⚠ 등)
- ⚠️ 병렬 세션이 /admin/mori(식단생성) 작업 중 — mori 파일은 이 세션에서 건드리지 않음. 식단생성 페이지 안에서 마감 여부 게이트(미마감 시 경고)는 추후 그 세션 작업 끝나면 추가 후보

## 2026-07-07 — 식단생성 ↔ 주간 명단 연동 (884e3a4)
- 식단생성(/admin/mori) 손님 목록이 주간 명단의 제외(meal_exclusions)를 반영 — 김나래 제외 시 식단생성에서도 빠짐 (5팀→4팀 검증 ✓)
- 식단생성 헤더에 명단 마감 상태 뱃지: ✅ 명단 마감됨·이름 / ⚠ 명단 미마감(클릭=명단으로 이동)
- 이상 비고 최종 규칙: 표시 주(월~일)에 발생 + 평일(월~금) 입·퇴소만 ⚠ (토·일 제외, 퇴소 주는 Last week로 충분)

## 2026-07-07 — 식단생성 점심=아카데미 월식단 연동 (3a17538, b254ebb)
- generate()/loadRound(재사용) 시 meal_menus(kind=academy, 월간) meal_data에서 해당 날짜 lunch를 점심에 자동 주입 → 라벨 "점심 (🎓 아카데미 월식단 · 수정 가능)" (칩 편집으로 매운 반찬 등 추가 가능). 식단 없으면 픽스 세트 폴백+안내
- 간식데이: 아카데미 월식단 snack 있으면 노란 배너로 표시 (조합 대상 아님)
- 그외 추천 "🔄 다른 추천" 버튼 — 그룹별 5개씩 로테이션. NEW/3d/3w/2mo 상대 표기
- 라이브 검증 ✓ (새로 조합 → 아카데미 라벨·간식데이 배너 표시, 저장 안 함)

## 2026-07-07 — 식단생성 추천 규칙 확정 (e92cebe)
- 그외 추천 = NEW + 회피기간 지난 메뉴만 (회피기간 내 제공 메뉴는 숨김 — 메이 확정). 라벨 NEW/3d/3w/2mo, 오래된 순, 🔄 다른 추천 로테이션
- 각 식단 항목 행에 🔄 버튼 = 그 메뉴만 교체 후보 모달 (행 클릭과 동일 기능을 눈에 보이게, 9694493)
- 참고: 이력 데이터는 확정된 회차부터 쌓임 — 초기엔 NEW가 많은 게 정상

## 2026-07-07 — 시재관리 하우스 번호 (d05efd4)
- cash_ledger.house_no 컬럼 추가 (DDL). 입금/출금 폼에 하우스 입력(자동완성 datalist 14룸: 17/7~17/18, 13/10, 16/19)
- 보증금 매칭 키 = 하우스+이름 (한쪽에 하우스 없으면 이름-only 폴백 — 과거 기록 호환). ref_id 정확 매칭은 최우선 그대로
- 보유현황 행에 하우스 뱃지, ↩반환→새 출금 생성 시 하우스 자동 복사
- 라이브 검증 ✓ (폼 하우스 입력+자동완성 14개, 보유현황 뱃지)

## 2026-07-07 — 시재 실사 정리 (데이터 작업, 코드 아님)
- 메이 지시: 실제 금고 잔액 ₱727, 보증금은 전부 전기세로 사용됨 → 기존 기록 보존한 채 정리 기록 추가
- 실행: 보유 보증금 9건(₱59,809) 각각 "보증금 전기세 사용 정리" 반환 출금(ref_id 정확 연결, 하우스 기록) + "시재 실사 조정" 입금 +₱62,490
- 결과 검증: 총 잔액 ₱727 ✓ · 보유 보증금 0건 ✓ (기록 전부 recorded_by=정리(Claude), 오늘 날짜)

## 2026-07-07 — 직원 달력 표기 최종 (882ab22)
- 체크인/체크아웃 → IN/OUT + 룸번호(B17L16)·리조트(JPARK/CUBE9/통학) 표기: "IN 윤영서, 윤준서/김미수 B17L16"

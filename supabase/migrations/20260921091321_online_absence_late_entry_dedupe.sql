...
begin
 ...
 -- Late entry of an earlier absence must not duplicate an already reported run.
 if exists(select 1 from public.staff_tasks t
   where t.application_source->>'type'='online_absence_followup'
   and t.application_source->>'enrollment_id'=e.id::text
   and exists(select 1 from jsonb_array_elements_text(t.application_source->'session_ids') x
     where x.value=any(streak_ids::text[]))) then return new; end if;

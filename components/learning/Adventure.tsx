...
import FinalMission, {Dreamy} from './FinalMission';
...
export default function Adventure(){
 ...
 return <main className={`${styles.adventure} ${motion?'':styles.still}`}>
  ...
  <section className={styles.theater} key={scene} aria-labelledby="scene-title">
   ...
   <div className={styles.dialogue}>{(scene===0||scene===8)&&<div className={styles.dreamyIntro}><Dreamy/><p><b>드림이</b>{scene===0?'안녕! 난 드림이야. 이야기 끝에서 우리 함께 미션도 해보자!':'잘 다녀왔지? 오늘 만난 친구들로 추억책을 만들어보자!'}</p></div>}<div className={styles.dialogueTop}><span>{s.speaker}</span><button onClick={()=>speaking?stop():speak(s.en)} disabled={busy} aria-label={speaking?'대사 멈추기':'대사 듣기'}><Icon name={speaking?'stop':'sound'} size={21}/></button></div><p className={styles.english}>{s.en.split(/(\b(?:tree|house|climb|ladder|knock|door|jungle|window)\b)/gi).map((part,i)=>{const index=data.words.findIndex(w=>w.word===part.toLowerCase());return index<0?<span key={i}>{part}</span>:<button className={styles.wordLink} disabled={busy} key={i} onClick={e=>openWord(index,e.currentTarget)} aria-label={`${part} 단어로 놀기`}>{part}</button>;})}</p><p className={styles.korean}>{s.ko}</p>
    {scene>0&&scene<8&&<div className={styles.wordTrail}><span>이 장면의 단어 친구 · 눌러서 함께 놀아요</span><div>{([[0,1],[0,1],[2,3],[4,5],[7,6],[6,7],[1,7],[0,1,2,3,4,5,6,7]][scene]).map(i=><button key={i} disabled={busy} onClick={e=>openWord(i,e.currentTarget)}><WordArt index={i} label=""/><b>{data.words[i].word}</b><Icon name="arrow" size={16}/></button>)}</div><small>듣기 · 그림 찾기 · 짝 맞추기 · 스펠링 · 쓰기 · 녹음</small></div>}
    ...
    {(scene===0||revealed||scene>=7)&&<button className={styles.continue} data-final-mission={scene===8?'true':undefined} disabled={busy} onClick={scene===8?()=>{stop();setMissionOpen(true);window.scrollTo({top:0,behavior:'instant'});}:next}>{scene===0?s.action:scene===7?s.action:scene===8?s.action:'이야기 계속하기'}<Icon name="arrow"/></button>}
  ...
  {scene===8&&<div className={styles.ending}><h2>이야기를 추억책으로 간직해요.</h2><p>드림이와 그림을 찾고, 짝을 맞추고, 나무집의 이름을 완성해봐요.</p><button className={styles.replay} onClick={()=>{stop();setScene(0);setRevealed(false);setTaps(0);window.scrollTo({top:0,behavior:'instant'});}}>이야기 다시 보기</button><Link href="/learn">이야기 서재로 돌아가기</Link></div>}
 </main>;
}

...
export default function Matching({index,onComplete,wordIds,initialMatched=[],onMatch,onMistake,showHeading=true}:{index:number;onComplete:()=>void;wordIds?:number[];initialMatched?:number[];onMatch?:(id:number)=>void;onMistake?:(id:number)=>void;showHeading?:boolean}){
 const ids=wordIds??[index,(index+1)%8,(index+3)%8];
 const [selected,setSelected]=useState<number|null>(null);
 const [matched,setMatched]=useState<number[]>(()=>initialMatched.filter(id=>ids.includes(id)));
 ...
 function choose(id:number){
  ...
  if(selected!==id){setMessage('아직 짝이 아니에요. 소리와 그림을 다시 살펴봐요.');onMistake?.(selected);return;}

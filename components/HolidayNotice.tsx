...
import { fmtHolidayList, HOLIDAY_NOTICE_LINES, HOLIDAY_NOTICE_LINES_ROOMONLY, type HolidayItem } from "@/lib/holidays";

type NoticeVariant = "package" | "roomonly";
const linesFor = (v?: NoticeVariant) => (v === "roomonly" ? HOLIDAY_NOTICE_LINES_ROOMONLY : HOLIDAY_NOTICE_LINES);

/* 기간 내 휴무일 안내 배너 */
export function HolidayBanner({ hits, variant }: { hits: HolidayItem[]; variant?: NoticeVariant }) {
  if (!hits.length) return null;
  const LINES = linesFor(variant);
  return (
...
export function HolidayPopup({ hits, onClose, variant }: { hits: HolidayItem[] | null; onClose: () => void; variant?: NoticeVariant }) {
  if (!hits) return null;
  const LINES = linesFor(variant);
  return (

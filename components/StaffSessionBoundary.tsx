...
export default function StaffSessionBoundary({children,requiredRole}:{children:React.ReactNode;requiredRole?:string}){
 ...
 useEffect(()=>{
  ...
  return()=>{clearInterval(timer);window.removeEventListener('focus',visible);window.removeEventListener('online',visible);document.removeEventListener('visibilitychange',visible);};
 },[check]);
 useEffect(()=>{
  if(state!=='signin'&&state!=='error')return;
 ...
 return <section style={{maxWidth:620,margin:'60px auto',padding:28,lineHeight:1.8,fontFamily:'Arial, sans-serif',color:'#243b53'}}>
  <h2>{state==='loading'?'화면을 불러오는 중입니다 · Loading':state==='signin'?'로그인 연결이 필요합니다 · Sign in':state==='forbidden'?'접근 권한 확인 · Access restricted':'서버 연결 확인 · Connection unavailable'}</h2>

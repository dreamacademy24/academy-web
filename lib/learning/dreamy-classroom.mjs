...
export function mountDreamyClassroom(canvas, { compact = false, onError = () => {}, onCharacterBounds } = {}) {
  ...
  function resize(width, height, pixelRatio = 1) {
    ...
    if (!disposed && onCharacterBounds) {
      // Project only the mascot, never its shadows or the whale. Measurements use
      // CSS pixels so the parent can place a native button without WebGL hit tests.
      camera.updateMatrixWorld(); mango.group.updateWorldMatrix(true, true);
      const box = new THREE.Box3().setFromObject(mango.group);
      const points = [];
      ...
      const left = Math.max(0, Math.min(...points.map(point => (point.x + 1) * width / 2)));
      const right = Math.min(width, Math.max(...points.map(point => (point.x + 1) * width / 2)));
      const top = Math.max(0, Math.min(...points.map(point => (1 - point.y) * height / 2)));
      const bottom = Math.min(height, Math.max(...points.map(point => (1 - point.y) * height / 2)));
      if (right > left && bottom > top) onCharacterBounds({ left, top, width: right - left, height: bottom - top });
    }
  }
  function dispose() {

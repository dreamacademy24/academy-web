...
export const PORTAL_FEATURES = [
  { key: "my_booking", label: "내 예약현황", desc: "예약·학생·결제 정보" },
  { key: "notices", label: "공지사항", desc: "안내·공지" },
  ...
  { key: "tutor", label: "튜터 수업", desc: "방문/아카데미 튜터 (신청·변경요청)" },
  { key: "online_class", label: "화상영어", desc: "기본: 체크인 60일 전 자동 오픈 · 켜면 즉시 오픈" },
  { key: "settlement", label: "정산내역", desc: "보증금·튜터비·추가비용 (기본 비활성)" },
  { key: "meal", label: "식단", desc: "아카데미 점심·드림하우스 식단" },
  { key: "my_applications", label: "내 신청 내역", desc: "셔틀/튜터/픽드랍 전체 신청" },
...
export function defaultPortalFeatures(b: BookingLike): PortalFeatureMap {
  ...
  if (cat.pkg === "올인원" || cat.pkg === "패키지") {
    return { checkin: !commute, shuttle: !commute, afterschool: !commute, tutor: true, meal: true, consultation: true, online_class: false, my_booking: true, notices: true, my_applications: true, settlement: false };
  }
  // 비패키지 드하+드아 (예: 장보운): 셔틀 O · 애프터스쿨 X · 튜터 O
  if (dhda) return { checkin: true, shuttle: true, afterschool: false, tutor: true, meal: true, consultation: true, online_class: false, my_booking: true, notices: true, my_applications: true, settlement: false };
  // 비패키지 통학형: 셔틀 X(패키지 전용) · 애프터스쿨 X(참여 불가, 필드트립은 관리자가 개별 오픈) · 튜터 O
  if (commute) return { checkin: false, shuttle: false, afterschool: false, tutor: true, meal: true, consultation: true, online_class: false, my_booking: true, notices: true, my_applications: true, settlement: false };
  // 비패키지 숙소 단독 (room only): 투어셔틀 X · 애프터스쿨 X · 튜터 O(리조트=아카데미 내)
  // 큐브나인은 저녁 식사 제공(2026-08-26~) → 식단 탭 O
  const isCube = (b?.accom_type || "").includes("큐브");
  return { checkin: true, shuttle: false, afterschool: false, tutor: true, meal: isCube, consultation: true, online_class: false, my_booking: true, notices: true, my_applications: true, settlement: false };
}

...
(function(){
  ...
  function preview(){
    ...
    if(!modulePromise)modulePromise=import('/staff-avatar-3d.js?v=20260924-2').catch(function(e){modulePromise=null;throw e;});
  ...
  function render(){
    ...
    host.innerHTML='<div class="avatar-nav" role="group" aria-label="프로필 설정 구분">'+[['avatar','🧸 아바타 꾸미기'],['theme','🎨 화면 색상'],['signature','✉️ 이메일 서명']].map(function(t){return '<button type="button" data-section="'+t[0]+'" aria-pressed="'+(section===t[0])+'">'+t[1]+'</button>';}).join('')+'</div><div id="avatarContent"></div><p id="avatarStatus" role="status"></p><div class="avatar-footer"><button type="button" id="avatarSave" class="btn btn-primary">'+(saved?'닫기':'저장하기')+'</button></div>';
    var content=host.querySelector('#avatarContent');
    if(section==='avatar')content.innerHTML='<h3>나만의 아바타 꾸미기</h3><p>할로윈 소품으로 작은 재미를 더해보세요. 이니셜이 함께 표시됩니다.</p><div class="avatar-room"><aside id="avatarPreview"></aside><div class="avatar-controls"><label>내 이니셜 <input id="avatarInitial" maxlength="2" value="'+safe(draft.initial)+'" aria-label="내 이니셜"></label><fieldset><legend>표시 방식</legend><div class="avatar-options">'+[['character','캐릭터 꾸미기'],['emoji','이모티콘'],['initial','이니셜']].map(function(t){return '<button type="button" data-mode="'+t[0]+'" aria-pressed="'+(draft.mode===t[0])+'">'+t[1]+'</button>';}).join('')+'</div></fieldset>'+(draft.mode==='character'?'<div class="avatar-category"><button type="button" data-category="basic" aria-pressed="'+(category==='basic')+'">기본 꾸미기</button><button type="button" data-category="season" aria-pressed="'+(category==='season')+'">🎃 시즌 소품 · 할로윈 (44)</button></div><p>'+(category==='season'?'모자·얼굴·의상·손·배경 장식을 함께 골라보세요. 저장한 모습은 시즌이 바뀌어도 유지됩니다.':'얼굴부터 옷, 배경까지 나만의 캐릭터를 만들어보세요.')+'</p>'+Object.keys(catalog).filter(function(k){return !!catalog[k].season===(category==='season');}).map(function(k){return options(k,catalog[k].title);}).join(''):draft.mode==='emoji'?'<div class="avatar-emoji">'+emojis.map(function(e,i){return '<button type="button" data-emoji="'+i+'" aria-label="'+e+' 선택" aria-pressed="'+(draft.emoji===e)+'">'+e+'</button>';}).join('')+'</div>':'')+'</div></div>';
    ...
    host.querySelectorAll('[data-piece]').forEach(function(b){b.onclick=function(){draft[b.dataset.piece]=b.dataset.value;delete draft.thumbnail;dirty();render();};});
  ...
  async function open(){
    ...
    try{var results=await Promise.all([request('/api/staff/avatar'),request('/api/staff/profile'),request('/api/staff/home-theme'),request('/staff-avatar-catalog.json')]);catalog=results[3];choices={};Object.keys(catalog).forEach(function(k){choices[k]=catalog[k].items;});if(stamp!==version||!CU||CU.id!==actor)return;
      people=results[0].avatars||{};var p=results[1].profile;draft=normalized(results[0].avatar||{initial:/^[A-Z0-9가-힣]{1,2}$/.test(p.initial)?p.initial:(actor==='song'?'S':actor==='vivace'?'V':'M'),mode:emojis.includes(p.initial)?'emoji':'initial',emoji:p.initial});profile={color:p.color||'#6366f1',signature:p.signature||'',theme:results[2].theme};render();

...
(function(){
  'use strict';
  var previousLook=null;var people={},draft=null,profile=null,version=0,section='avatar',saved=false,busy=false;
  ...
  function preview(){
    ...
    if(!modulePromise)modulePromise=import('/staff-avatar-3d.js?v=20260924-hair2').catch(function(e){modulePromise=null;throw e;});
  ...
  var looks=[
 {name:'☕ 카페 산책',values:{hair:'bob',bangs:'curtain',clothing:'knit',outfitColor:'cream',background:'peach',accessory:'glasses',handProp:'coffee',pet:'cat',shoes:'sneakers'}},
 {name:'🎧 스트리트',values:{hair:'pixie',hairColor:'silver',clothing:'hoodie',outfitColor:'black',background:'sky',accessory:'headphones',handProp:'phone',shoes:'platform',shoeColor:'white'}},
 {name:'🌷 꽃 피는 정원',values:{hair:'long',bangs:'side',clothing:'dress',outfitColor:'pink',background:'mint',earrings:'pearl',handProp:'flower',backgroundProp:'plant',pet:'rabbit',shoes:'ballet'}},
 {name:'📚 오늘의 매니저',values:{hair:'bun',clothing:'shirt',outfitColor:'blue',background:'lavender',accessory:'glasses',handProp:'book',neckwear:'pendant',shoes:'classic'}},
 {name:'🎃 꼬마 마법사',values:{hair:'curl',clothing:'dress',background:'night',seasonHat:'witch',seasonHand:'wand',seasonBackground:'spider',seasonOutfit:'witchrobe',pet:'cat'}},
 {name:'🦇 달빛 파티',values:{hair:'short',hairColor:'purple',outfitColor:'red',background:'night',seasonHat:'horns',seasonFace:'fangs',seasonHand:'lantern',seasonBackground:'bat',seasonOutfit:'vampirecape'}}
 ];
 function rememberLook(){previousLook=Object.assign({},draft);}
 function propTray(){var keys=selectedProps();return '<div class="avatar-prop-tray"><strong>착용 소품 '+keys.length+'/5개</strong><p>기본 꾸미기는 개수에 포함되지 않습니다. 소품은 종류별 하나씩, 모두 합쳐 5개까지 조합합니다.</p><div class="avatar-options">'+keys.map(function(k){return '<button type="button" data-remove-prop="'+k+'" aria-label="'+safe(choices[k][draft[k]])+' 빼기">'+safe(choices[k][draft[k]])+' ×</button>';}).join('')+'</div><span id="avatarPropLimit" role="status">'+(keys.length>5?'소품을 5개 이하로 줄인 뒤 저장해주세요.':'')+'</span></div>';}
  function render(){
    ...
    if(section==='avatar')content.innerHTML='<h3>나만의 아바타 꾸미기</h3><p>할로윈 소품으로 작은 재미를 더해보세요. 이니셜이 함께 표시됩니다.</p><div class="avatar-room"><aside id="avatarPreview"></aside><div class="avatar-controls"><label>내 이니셜 <input id="avatarInitial" maxlength="2" value="'+safe(draft.initial)+'" aria-label="내 이니셜"></label><fieldset><legend>표시 방식</legend><div class="avatar-options">'+[['character','캐릭터 꾸미기'],['emoji','이모티콘'],['initial','이니셜']].map(function(t){return '<button type="button" data-mode="'+t[0]+'" aria-pressed="'+(draft.mode===t[0])+'">'+t[1]+'</button>';}).join('')+'</div></fieldset>'+(draft.mode==='character'?'<details class="avatar-lookbook"><summary>✨ 추천 코디 · 한 번에 입어보기</summary><div class="avatar-options">'+looks.map(function(l,i){return '<button type="button" data-look="'+i+'">'+l.name+'</button>';}).join('')+'</div></details><div class="avatar-options"><button type="button" data-random-look>🎲 랜덤 꾸미기</button><button type="button" data-undo-look '+(!previousLook?'disabled':'')+'>↶ 이전 모습</button></div>'+propTray()+'<div class="avatar-category"><button type="button" data-category="basic" aria-pressed="'+(category==='basic')+'">기본 꾸미기</button><button type="button" data-category="season" aria-pressed="'+(category==='season')+'">🎃 시즌 소품 · 할로윈 (44)</button></div><p>'+(category==='season'?'모자·얼굴·의상·손·배경 장식을 함께 골라보세요. 저장한 모습은 시즌이 바뀌어도 유지됩니다.':'얼굴부터 옷, 배경까지 나만의 캐릭터를 만들어보세요.')+'</p>'+(category==='basic'?'<div class="avatar-category">'+Object.keys(groups).map(function(k){return '<button type="button" data-group="'+k+'" aria-pressed="'+(group===k)+'">'+groups[k][0]+'</button>';}).join('')+'</div>':'')+Object.keys(catalog).filter(function(k){return category==='season'?!!catalog[k].season:groups[group].includes(k);}).map(function(k){return options(k,catalog[k].title);}).join(''):draft.mode==='emoji'?'<div class="avatar-emoji">'+emojis.map(function(e,i){return '<button type="button" data-emoji="'+i+'" aria-label="'+e+' 선택" aria-pressed="'+(draft.emoji===e)+'">'+e+'</button>';}).join('')+'</div>':'')+'</div></div>';
    ...
    setupTabs();
    preview();
    host.querySelectorAll('[data-look]').forEach(function(b){b.onclick=function(){rememberLook();Object.keys(catalog).forEach(function(k){draft[k]=catalog[k].default||defaults[k];});Object.assign(draft,looks[Number(b.dataset.look)].values);delete draft.thumbnail;dirty();render();};});
    var random=host.querySelector('[data-random-look]');if(random)random.onclick=function(){rememberLook();Object.keys(catalog).forEach(function(k){var values=Object.keys(choices[k]);draft[k]=catalog[k].prop?'none':values[Math.floor(Math.random()*values.length)];});var props=Object.keys(catalog).filter(function(k){return catalog[k].prop;});for(var i=props.length-1;i>0;i--){var j=Math.floor(Math.random()*(i+1)),tmp=props[i];props[i]=props[j];props[j]=tmp;}props.slice(0,3).forEach(function(k){var values=Object.keys(choices[k]).filter(function(v){return v!=='none';});draft[k]=values[Math.floor(Math.random()*values.length)];});delete draft.thumbnail;dirty();render();};
    var undo=host.querySelector('[data-undo-look]');if(undo)undo.onclick=function(){if(!previousLook)return;var current=Object.assign({},draft);draft=previousLook;previousLook=current;delete draft.thumbnail;dirty();render();};
    ...
    host.querySelectorAll('[data-piece]').forEach(function(b){b.onclick=function(){var k=b.dataset.piece,v=b.dataset.value;if(catalog[k].prop&&v!=='none'&&draft[k]==='none'&&selectedProps().length>=5){document.getElementById('avatarPropLimit').textContent='소품은 최대 5개입니다. 선택한 소품의 ×를 눌러 하나를 빼주세요.';return;}rememberLook();draft[k]=v;delete draft.thumbnail;dirty();render();};});
    host.querySelectorAll('[data-remove-prop]').forEach(function(b){b.onclick=function(){rememberLook();draft[b.dataset.removeProp]='none';delete draft.thumbnail;dirty();render();};});
  ...
  async function open(){
    var host=document.getElementById('sp5');if(!host||!CU)return;var stamp=++version,actor=CU.id;saved=false;busy=false;draft=null;profile=null;previousLook=null;
  ...
  var css=document.createElement('link');css.rel='stylesheet';css.href='/staff-avatar.css?v=20260924-hair2';document.head.append(css);
  function setupTabs(){

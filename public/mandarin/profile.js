...
function choose(id){if(!['jia','jiwoo'].includes(id))return;activeProfile=id;const name=id==='jia'?'jia':'jiu';stars=new Set();try{const list=JSON.parse(localStorage.getItem('little-mandarin-stars-v1-'+id)||'[]');if(Array.isArray(list))stars=new Set(list.filter(x=>WORDS.some(w=>w.id===x)));}catch{}
